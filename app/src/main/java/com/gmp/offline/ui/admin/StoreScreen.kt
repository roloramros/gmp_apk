...
// Wrapper de "Tienda" para admin y comercial (ver comentario equivalente
// en CatalogScreen.kt) — llegan acá desde el ítem "Tienda" del drawer.
...
fun StoreScreen(onBack: () -> Unit) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { if (searchVisible) SearchField(searchQuery, { searchQuery = it }) else Text("Tienda") },
                navigationIcon = {
        ...
        StoreTabContent(searchQuery = searchQuery, modifier = Modifier.padding(padding))
    }
}
